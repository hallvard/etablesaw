...
import etablesaw.ui.BooleanColumnFilterSupport;
import etablesaw.ui.ExprViewerRowFilterSupport;
import etablesaw.ui.TableProvider;
import etablesaw.ui.TableProviderHelper;
import etablesaw.ui.TablesawContentRow;
...
public class TablesawEditor extends EditorPart implements TableProvider, ISelectionProvider {
	...
	public void dispose() {
		...
		if (input instanceof IFileEditorInput) {
			final IFile file = ((IFileEditorInput) input).getFile();
			Activator.getInstance().getTableProviderRegistry().registerTableProvider(file.getName(), null);
		}
		super.dispose();
	}
		...
		if (modelTable != null) {
			tablesawViewer.setContentProvider(new AbstractTablesawContentProvider() {
				...
				protected boolean isContentTable(final Table table) {
					return TablesawEditor.this.isContentTable(table);
				}
			});
		}
				...
				protected void updateViewerAfterEdit(final Object element) {
					...
					if (element instanceof TablesawContentRow) {
						...
						if (table == columnFilterTable) {
							tableProviderHelper.fireTableChanged(TablesawEditor.this);
						} else if (table == rowFilterTable) {
							exprViewerRowFilterSupport.updateViewerFilter();
							filterViewer();
							tableProviderHelper.fireTableDataChanged(TablesawEditor.this);
